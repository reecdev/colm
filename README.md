...
How it works: Type a question in the sidebar, the agent thinks, creates code cells in the notebook, runs them, and shows you the results â all without leaving the page.
...
Stack: Node/Express/WebSockets backend â React/CodeMirror 6 frontend â Python subprocess for kernel. Bundled with esbuild.
To run: npm install && npm run build && node bin/cli.js â needs OPENROUTER_API_KEY in env.
